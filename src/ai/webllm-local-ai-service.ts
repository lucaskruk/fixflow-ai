...
export { buildDiagnosisRequestContent } from "./ai-prompts";

export type LocalAIPhase = AIPhase;
export type LocalAIRuntimeSnapshot = AIRuntimeSnapshot<LocalAIModelId>;
export type LocalAIDebugOutput = AIDebugOutput;
export type LocalAIFailure = AIFailure;
...
export class WebLLMLocalAIService implements LocalAIService {
  ...
  constructor(modelId: LocalAIModelId = loadSelectedLocalAIModelId()) {
    const selectedModel = getLocalAIModel(modelId);
    this.snapshot = {
      provider: "local",
      phase: "checking",
      progress: 0,
